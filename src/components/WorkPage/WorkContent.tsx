...
import NiceBizinfo from "./Pages/NiceBizinfo";
...
const WorkContent: React.FC<WorkContentProps> = ({ item }) => {
	const renderContent = () => {
		switch (item.id) {
			case "nicebizinfo":
				return <NiceBizinfo item={item} />;
			case "nhopen":
				return <Nhopensource item={item} />;
			case "portfolio":
				return <Portfoliopage item={item} />;
			case "knou":
				return <Knou item={item} />;
			case "u":
				return <U item={item} />;
			case "kepco":
				return <Kepco item={item} />;
			case "gachon":
				return <Gachon item={item} />;
			case "hira":
				return <Hira item={item} />;
			case "yu":
				return <Yu item={item} />;
			case "catholic":
				return <Catholic item={item} />;
			default:
				return <p>기본 설명입니다.</p>;
		}
	};
